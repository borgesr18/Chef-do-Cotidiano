// /app/certificados/id/[id]/page.tsx
import { supabase } from '@/lib/supabaseClient'
import { notFound } from 'next/navigation'
import QRCode from 'qrcode.react'

type CertificadoPublicoData = {
  user: {
    nome: string
  }
  curso: {
...
export default async function CertificadoPublico({
  ...
  const urlPublica = `${process.env.NEXT_PUBLIC_BASE_URL}/certificados/id/${certificadoId}`

  const { data: registro, error } = await supabase
    .from('certificados_publicos')
    .select('user:nome, curso:titulo, curso:autor, criado_em')
    .eq('id', certificadoId)
    .single<CertificadoPublicoData>()
  ...
  return (
    <div className="p-10 max-w-3xl mx-auto border border-gray-400 shadow-xl rounded text-center bg-white">
      ...
      <p className="text-gray-600">
        Autor: {registro.curso.autor || 'Desconhecido'}
      </p>
      <p className="mt-6 text-sm text-gray-500">
        Emitido em {new Date(registro.criado_em).toLocaleDateString('pt-BR')}
      </p>
